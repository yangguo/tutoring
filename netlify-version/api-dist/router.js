...
const lesson_js_1 = __importDefault(require("./chat/lesson.js"));
const speaking_practice_js_1 = __importDefault(require("./chat/speaking-practice.js"));
const router = (0, express_1.Router)();
const INLINE_IMAGE_MAX_BYTES = 10 * 1024 * 1024; // 10MB safety limit for inline images
const DEFAULT_OPENAI_VISION_TIMEOUT_MS = 180000; // 3 minutes for potentially slower APIs
async function getInlineImageUrl(imageUrl) {
    try {
...
// Achievements Routes
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
const awardAchievementSchema = joi_1.default.object({
    userId: joi_1.default.string().uuid().required(),
    achievementId: joi_1.default.string().uuid().required()
});
router.get('/achievements', async (_req, res) => {
    try {
        const { data: achievements, error } = await supabase_js_1.supabase
            .from('achievements')
            .select('*')
...
router.get('/achievements/user/:userId', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { data: achievements, error: achievementsError } = await supabase_js_1.supabase
            .from('achievements')
            .select(`*,
...
router.post('/achievements/award', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['admin']), async (req, res) => {
    try {
        ...
        const { data: achievement, error: achievementError } = await supabase_js_1.supabase
            .from('achievements')
            .select('*')
            .eq('id', achievementId)
            .single();
        ...
        const { data: existingAward, error: existingError } = await supabase_js_1.supabase
        ...
        const { data: userAchievement, error: awardError } = await supabase_js_1.supabase
            .from('user_achievements')
            .insert({
            user_id: userId,
...
router.delete('/achievements/revoke', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['admin']), async (req, res) => {
    try {
        const { error: validationError, value } = awardAchievementSchema.validate(req.body);
        if (validationError) {
            return res.status(400).json({ error: validationError.details[0].message });
        ...
        const { data: deletedAchievement, error: deleteError } = await supabase_js_1.supabase
            .from('user_achievements')
            .delete()
            .eq('user_id', userId)
            .eq('achievement_id', achievementId)
...
router.get('/achievements/leaderboard', async (req, res) => {
    try {
        ...
        const { data: leaderboard, error } = await supabase_js_1.supabase
            .from('users')
            .select(`id,
        username,
        avatar_url,
        ...
        const leaderboardWithPoints = leaderboard.map(user => {
            const totalPoints = user.user_achievements.reduce((sum, ua) => {
...
const registerSchema = joi_1.default.object({
    ...
    role: joi_1.default.string().valid('child', 'parent', 'admin').required(),
    age: joi_1.default.number().min(3).max(18).when('role', {
        is: 'child',
        then: joi_1.default.required(),
        otherwise: joi_1.default.optional()
    }),
    grade_level: joi_1.default.string().when('role', {
        is: 'child',
        then: joi_1.default.optional(),
        otherwise: joi_1.default.forbidden()
    }),
    parent_email: joi_1.default.string().email().when('role', {
        is: 'child',
        then: joi_1.default.optional(),
        otherwise: joi_1.default.forbidden()
    })
...
const updateProfileSchema = joi_1.default.object({
    ...
    age: joi_1.default.number().min(3).max(18).optional(),
    grade_level: joi_1.default.string().optional()
});
router.post('/auth/register', async (req, res) => {
    try {
        ...
        const { data: authData, error: authError } = await supabase_js_1.supabase.auth.admin.createUser({
            email,
            password,
            email_confirm: true
        });
        ...
        if (parent_email && role === 'child') {
            const { data: parentData } = await supabase_js_1.supabase
                .from('users')
                .select('id')
        ...
        // Generate JWT token
        const token = (0, jwt_js_1.generateToken)(userData);
        res.status(201).json({
...
router.post('/auth/logout', jwt_js_1.authenticateToken, async (_req, res) => {
    try {
...
router.get('/auth/me', jwt_js_1.authenticateToken, async (req, res) => {
    try {
...
router.put('/auth/profile', jwt_js_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/auth/children', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['parent']), async (req, res) => {
    try {
        const { userId } = req.user;
...
// Books Routes
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
const readingSessionSchema = joi_1.default.object({
    book_id: joi_1.default.string().uuid().required(),
    pages_read: joi_1.default.array().items(joi_1.default.number().positive()).required(),
    time_spent: joi_1.default.number().positive().required(),
    comprehension_score: joi_1.default.number().min(0).max(100).optional(),
    vocabulary_learned: joi_1.default.array().items(joi_1.default.string().uuid()).optional()
});
router.get('/books', async (req, res) => {
    try {
        const { page = 1, limit = 12, difficulty, category, target_age, search } = req.query;
...
router.get('/books/discussions', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
...
router.get('/books/:bookId', async (req, res) => {
    try {
        ...
        const { data: book, error: bookError } = await supabase_js_1.supabase
            .from('books')
            .select('*')
            .eq('id', bookId)
            .single();
        ...
        if (!allowed && userId && book.uploaded_by === userId)
            allowed = true;
        if (!allowed && userId) {
            const { data: lessons } = await supabase_js_1.supabase
                .from('lesson_plans')
                .select('id')
        ...
        // Get book pages
        const { data: pages, error: pagesError } = await supabase_js_1.supabase
            .from('book_pages')
            .select('*')
            .eq('book_id', bookId)
...
router.post('/books/reading-session', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { data: sessionData, error: sessionError } = await supabase_js_1.supabase
...
router.get('/books/progress', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
...
router.get('/books/reading-sessions', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
...
router.get('/books/speaking-sessions', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
        const { book_id, page = 1, limit = 10 } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
...
router.get('/books/vocabulary', async (req, res) => {
    try {
...
router.post('/books/vocabulary/learn', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
        ...
        const { data: word, error: wordError } = await supabase_js_1.supabase
            .from('vocabulary_words')
            .select('id')
            .eq('id', word_id)
            .single();
        ...
        const { data: userVocab, error: vocabError } = await supabase_js_1.supabase
            .from('user_vocabulary')
            .insert({
            user_id: userId,
            word_id,
...
router.get('/books/vocabulary/learned', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
        const { page = 1, limit = 20 } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        const { data: learnedWords, error } = await supabase_js_1.supabase
            .from('user_vocabulary')
            .select(`*,
        vocabulary_words (
          id,
...
router.post('/books/evaluate-pronunciation', jwt_js_1.authenticateToken, async (req, res) => {
    try {
...
function evaluateBasicPronunciation(transcript, targetText, confidence) {
...
router.post('/books/analyze-image', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const hasValidOpenAIConfig = Boolean(process.env.OPENAI_BASE_URL &&
            process.env.OPENAI_API_KEY &&
            process.env.OPENAI_API_KEY !== 'your-openai-api-key-here' &&
            ...
            try {
                ...
                try {
                    ...
                    if (!openaiResponse.ok) {
                        ...
                        const extractContentString = (content) => {
                            if (!content)
                                return null;
...
router.post('/books/:bookId/analyze-images', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['admin']), async (req, res) => {
    try {
        const { bookId } = req.params;
...
function generateBasicImageDescription(_imageUrl, context) {
    ...
    return contextDescriptions[contextKey] || contextDescriptions.default;
}
router.post('/books/extract-vocabulary', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        for (const vocab of extractedVocabulary) {
            try {
                ...
                const { data: existingWord } = await supabase_js_1.supabase
                    .from('vocabulary_words')
                    .select('id')
                ...
                if (!existingWord) {
                    ...
                    const { data: newWord, error: insertError } = await supabase_js_1.supabase
                        .from('vocabulary_words')
                        .insert({
...
function extractBasicVocabulary(description, difficultyLevel, maxWords) {
...
router.post('/books/discuss', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        let pageContext = '';
        if (page_number) {
            const { data: page, error: pageError } = await supabase_js_1.supabase
                .from('book_pages')
                .select('*')
                .eq('book_id', book_id)
...
function generateFallbackDiscussionResponse(message, book, _pageContext) {
...
// Chat Routes
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
router.post('/chat/lesson', ...lesson_js_1.default);
router.post('/chat/speaking-practice', ...speaking_practice_js_1.default);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Dashboard Routes
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
router.use('/dashboard', jwt_js_1.authenticateToken);
router.get('/dashboard/students', async (_req, res) => {
    try {
        const { data: students, error } = await supabase_js_1.supabase
            .from('users')
            .select(`id,
        full_name,
        email,
...
router.get('/dashboard/lessons', async (_req, res) => {
    try {
        const { data: lessons, error } = await supabase_js_1.supabase
            .from('lesson_plans')
            .select('*')
            .order('created_at', { ascending: false });
        if (error) {
...
router.get('/dashboard/my-lessons', async (req, res) => {
    try {
        ...
        const transformedLessons = await Promise.all((lessons || []).map(async (lesson) => {
            ...
            if (lesson.book_ids && lesson.book_ids.length > 0) {
                const { data: bookData, error: bookError } = await supabase_js_1.supabase
                    .from('books')
...
router.post('/dashboard/lessons', async (req, res) => {
    try {
        ...
        const { data: lesson, error } = await supabase_js_1.supabase
            .from('lesson_plans')
            .insert({
            title,
            description,
...
router.put('/dashboard/lessons/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, targetLevel, duration, objectives, activities, assignedStudents, bookIds } = req.body;
        const updateData = {};
        ...
        if (targetLevel !== undefined)
            updateData.target_level = targetLevel;
        ...
        if (assignedStudents !== undefined)
            updateData.assigned_students = assignedStudents;
        if (bookIds !== undefined)
            updateData.book_ids = bookIds;
        const { data: lesson, error } = await supabase_js_1.supabase
            .from('lesson_plans')
            .update(updateData)
            .eq('id', id)
...
router.delete('/dashboard/lessons/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/dashboard/progress', async (_req, res) => {
    try {
        const { data: readingSessions, error: readingError } = await supabase_js_1.supabase
            .from('reading_sessions')
            .select('duration, words_read, comprehension_score, created_at')
            .order('created_at', { ascending: false })
            .limit(10);
        const { data: speakingSessions, error: speakingError } = await supabase_js_1.supabase
            .from('speaking_sessions')
            .select('pronunciation_score, fluency_score, accuracy_score, created_at')
            .order('created_at', { ascending: false })
            .limit(10);
        const { data: vocabularyCount, error: vocabError } = await supabase_js_1.supabase
            .from('user_vocabulary')
...
router.get('/dashboard/analytics', async (_req, res) => {
    try {
        ...
        const { data: speakingSessions, error: speakingError } = await supabase_js_1.supabase
            ...
            .gte('created_at', thirtyDaysAgo.toISOString())
            .order('created_at', { ascending: true });
        const { data: vocabularyData, error: vocabError } = await supabase_js_1.supabase
            .from('user_vocabulary')
            .select('learned_at')
...
// Regenerate Routes
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
router.post('/books/:bookId/pages/:pageId/regenerate-description', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { data: updatedPage, error: updateError } = await supabase_js_1.supabase
            .from('book_pages')
...
const bookMetadataSchema = joi_1.default.object({
    title: joi_1.default.string().required(),
    description: joi_1.default.string().optional(),
    target_age_min: joi_1.default.number().integer().min(3).max(18).required(),
    target_age_max: joi_1.default.number().integer().min(3).max(18).required(),
...
router.post('/upload/book', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['parent', 'admin']), upload.single('file'), handleMulterError, async (req, res) => {
    try {
        ...
        const { data: _uploadData, error: uploadError } = await supabase_js_1.supabase.storage
            .from('book-files')
        ...
        const { data: urlData } = supabase_js_1.supabase.storage
            .from('book-files')
        ...
        if (bookError) {
            ...
            await supabase_js_1.supabase.storage.from('book-files').remove([filePath]);
...
router.patch('/upload/book/:bookId', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['parent', 'admin']), async (req, res) => {
    try {
        const { bookId } = req.params;
        ...
        const updateSchema = joi_1.default.object({
            ...
            target_age_min: joi_1.default.number().integer().min(3).max(18).optional(),
            target_age_max: joi_1.default.number().integer().min(3).max(18).optional(),
            difficulty_level: joi_1.default.string().valid('beginner', 'intermediate', 'advanced').optional()
        }).min(1);
        ...
        const { data: book, error: fetchError } = await supabase_js_1.supabase
            .from('books')
        ...
        const { data: updated, error: updateError } = await supabase_js_1.supabase
            .from('books')
            .update({ ...updates })
            .eq('id', bookId)
            .select('*')
...
router.post('/upload/book/:bookId/pages', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['parent', 'admin']), upload.array('pages', 50), async (req, res) => {
    try {
        const { bookId } = req.params;
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                // Create page record
                const { data: pageData, error: pageError } = await supabase_js_1.supabase
                    .from('book_pages')
                    .insert({
                    book_id: bookId,
                    page_number: i + 1,
...
router.get('/upload/books', jwt_js_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.user;
        const { page = 1, limit = 10 } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        const { data: books, error } = await supabase_js_1.supabase
            .from('books')
            .select('*')
...
router.delete('/upload/book/:bookId', jwt_js_1.authenticateToken, (0, jwt_js_1.requireRole)(['parent', 'admin']), async (req, res) => {
    try {
        const { bookId } = req.params;
        const { userId } = req.user;
        // Verify book exists and user has permission
        ...
        const { data: pages } = await supabase_js_1.supabase
            .from('book_pages')
            .select('image_path')
            .eq('book_id', bookId);
        ...
        const { error: deleteError } = await supabase_js_1.supabase
            .from('books')
            .delete()
            .eq('id', bookId);
        if (deleteError) {
